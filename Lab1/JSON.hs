...
-- toString (JObject o@(x@(title, value):xs)) = title ++ " " ++ toString value ++ ", " ++ list2str(xs)
toString (JObject o)    = "(" ++ tuple2str o ++ ")"
...
tuple2str :: [(String,JValue)] -> String
tuple2str [] = ""
tuple2str (x@(title,value):[]) = title ++ " " ++ toString value
tuple2str (x@(title,value):xs) = title ++ " " ++ (toString value) ++ ", " ++ (tuple2str xs)
