...
testUnit = do
  ...
  putStrLn $ show $ WhileInterp.run (Sequence
                                        ...
                                        (Var "X"))
  putStrLn $ show $ WhileInterp.run (Op1 Not (Val (BoolVal True)))
  -- Should be: (BoolVal False,fromList [])
  putStrLn $ show $ WhileInterp.run (Op And (Val (BoolVal True)) (Val (BoolVal False))) 
  -- Should be: (BoolVal False,fromList [])
  putStrLn $ show $ WhileInterp.run (Op Or (Val (BoolVal False)) (Val (BoolVal False)))
  -- Should be: (BoolVal False,fromList [])
